    ...
    axios.get(url, { rejectUnauthorized: false })
      .then((response) => {
        if (response.status === 200) {
          this.log.info(`${switchConfig.name} toggled successfully`); //log successful toggles
          callback(null);
        } else {
          this.log.warn(`ERROR SETTING ${switchConfig.name}, CODE: ${response.status}`); //log any errors from unexpected response
          callback(new Error(`Invalid response: ${response.status}`));
        }
      })
      .catch((error) => {
        this.log.warn(`ERROR SETTING ${switchConfig.name}: ${error}`); //log any errors from no response
        callback(error);
      });
  }
    ...
    axios.get(this.config.statusUrl, { rejectUnauthorized: false })
      .then((response) => {
        if (response.status === 200) {
          ...
          this.log.info(`Status Request: ${this.config.statusUrl}`); //log successful status requests
          callback(null, isOn);
        } else {
          this.log.warn(`REQUEST ERROR: ${this.config.statusUrl}, CODE: ${response.status}`); //log any errors from unexpected response
          callback(new Error(`Invalid response: ${response.status}`));
        }
      })
      .catch((error) => {
        this.log.warn(`REQUEST ERROR: ${this.config.statusUrl}: ${error}`); //log any errors from no response
        callback(error);
      });
  }
